...
module.exports = {
  ...
  create(context) {
    ...
    function isUnderscored(name) {
      // skip UNSAFE_ beginning
      if (name.substring(0, 7) === 'UNSAFE_') {
        ...
        if (node.parent.type === 'MemberExpression') {
          ...
          if (
            ...
            (effectiveParent.right.type !== 'MemberExpression' ||
              (effectiveParent.left.type === 'MemberExpression' && effectiveParent.left.property.name === node.name))
          ) {
            report(node);
          }
          ...
          ['ImportSpecifier', 'ImportNamespaceSpecifier', 'ImportDefaultSpecifier'].indexOf(node.parent.type) >= 0
        ) {
